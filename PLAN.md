...
- **Repo:** `github.com/shengjianpeng1987/citation-verify`. Public from 2026-04-25. **Phase 0 amendment 2 (2026-04-25):** original plan was "private through Phase 4; flipped public at start of Phase 5". JOSS 2026 policy requires ≥6 months of public development history before submission, so the repo went public during Phase 3 to start the clock. The remaining two user gates (`v1.0.0` tag → Zenodo DOI; JOSS submission itself) are unchanged and still wait until Phase 5 per Execution rule 2.
...
- `docx_patch.py` — deferred from Phase 1 (formerly an unimplemented `--apply-replacements` promise in SKILL.md). **Status (2026-04-25):** unblocked — `synthetic_mixed.replacements[]` now populated (`tests/snapshots/synthetic_mixed/replacements.json`, citation #5 → Doench 2016, fit_score=8) so the test data dependency is satisfied. Scheduling deferred to v0.3+, post-JOSS submission per the Phase 3 sequencing decision. Acceptance:
  - (a) At least 1 Phase 3 case has non-empty `replacements[]` with a `confirmed_replacement` landing as `canonical_record`. **Met by `synthetic_mixed`.**
...
### Cowork/Claude tasks
- Draft `paper/paper.md` (≤1000 words target, JOSS hard practical limit). **Eight sections per the JOSS 2026 spec** (Phase 0 amendment 2026-04-25: previous five-section outline used the pre-2026 template):
  1. **Summary** (~120 words) — what citation-verify does in neutral technical language. The headline contribution is the dual-channel reverse-validation architecture, not any individual stage.
  2. **Statement of need** (~250 words, two paragraphs). Paragraph 1: problem — LLM-assisted writing has made citation hallucination a new academic integrity risk, particularly in biomedical reviews where AI drafting is common. Paragraph 2: gap — no published workflow explicitly partitions deterministic API adjudication and exploratory LLM probing, joined by a reverse-validation invariant preventing LLM outputs from becoming canonical records without API confirmation.
  3. **State of the field** (~150 words) — survey of existing approaches and how citation-verify is positioned. Coverage: (a) DOI-lookup tools (scite.ai, Semantic Scholar paper validation) that verify existence but not claim-to-source fidelity; (b) LLM self-check approaches (recent benchmarks show recall <60% on adversarial inputs); (c) manual reviewer practice. Position citation-verify as the complementary tool that enforces the deterministic / LLM partition rather than choosing one side.
  4. **Software design** (~300 words) — reference Figure 1; name the canonical_record invariant; describe the Stage 4b corrections / replacements / unresolvable split; explain how the snapshot harness combines recorded fixtures (real Codex on real research docs, local-only) and hand-crafted fixtures (deterministic CI contract). Reference Figure 2 for a worked example: the EIF4ENIF1 case where five real papers had their author bylines drifted to fabricated names; Channel A's textual-similarity scoring alone would have approved them, but the metric-honest Stage 4b output flagged all five as `corrections[]` with `requires_human_review=true` via the deterministic backstop.
  5. **Research impact statement** (~150 words) — explicit ties to scenarios citation-verify catches that prior tools do not: per-citation field-level drift (Phase 1 nasal-methylation case had four such items), and the asymmetric-overlap blindspot (Phase 2 EIF4ENIF1 case revealed Channel A's `author_overlap=0.95` was a DOI-floor artifact). Argue that the architecture is reusable for any domain that has both an API of record and an LLM-fallible knowledge claim.
  6. **AI usage disclosure** (~100 words) — JOSS 2026 mandate. Disclose: (a) AI assistants (Claude Code / Cowork) participated in code generation, planning, and review throughout Phases 0–4. Every commit was authored by the human PI under their git identity; AI co-authorship is recorded in `Co-Authored-By` trailers. (b) AI is also a runtime component: `codex_atom.sh` invokes the Codex CLI for Channel B verification and Stage 4 replacement search. (c) Final acceptance of every claim in the report (verdicts, corrections, replacements) is the user's responsibility; the tool surfaces evidence and disagreements but does not act unilaterally.
  7. **Acknowledgements** — funding (if any), institutional support (NUAA), prior work (the JOSS reviewers whose policy revisions shaped the disclosure norms in this paper).
  8. **References** — populated from `paper/paper.bib`.
